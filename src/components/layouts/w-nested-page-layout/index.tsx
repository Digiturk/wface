...
export interface WNestedPageLayoutProps extends WithStyles<typeof styles> {
  ...
  design?: {
    navigationButton?: SxProps;
    navigationPopper?: SxProps;
  };
}

export interface WNestedPageLayoutState {
  stack: {
    item?: WNestedPageItem;
    payload: any;
...
export class WNestedPageLayoutInner extends React.Component<
  WNestedPageLayoutProps,
  WNestedPageLayoutState
> {
  constructor(props: WNestedPageLayoutProps) {
    super(props);
          ...
          this.setState(
            {
              stack: [
                ...this.state.stack,
                { item, payload, ref: React.createRef() },
              ],
            },
            () => this.onStateChanged()
          );
              ...
              return (
                <div style={{ display: "inline-block" }}>
                  <WButton
                    ...
                    sx={{
                      ...this.props.design?.navigationButton,
                    }}
                    onClick={() => {
                  ...
                  {index !== this.state.stack.length - 1 && (
                    <>
                      <WIconButton
                        ...
                        sx={{ ...this.props.design?.navigationPopper }}
                        aria-owns={
                        ...
                        style={{
                          transform:
                            this.state.anchorEl &&
                            this.state.openMenuIndex === index
                              ? "rotate(90deg)"
                              : "none",
                        }}
                        color="primary"
                        icon="chevron_right"
                        disabled={(current?.item?.subItems?.length || 0) < 2}
                        onClick={(event) =>
                          this.setState(
                            {
                                  ...
                                  {current?.item?.subItems?.map((item) => (
                                    <WMenuItem
                                      ...
                                      onClick={() => {
                                        if (
                                          item.name ===
                                          this.state.stack[index + 1]?.item?.name
                                        ) {
                                          this.setState(
                                            {
                                              anchorEl: null,
                                    ...
                                    >
                                      {this.getTitle({
                                        item,
                                        payload: this.state.stack[index + 1]
                                          .payload,
                                      })}
                                    </WMenuItem>
                                  ))}
...
export const WNestedPageLayout = withStyles(styles, {
  withTheme: true,
})((props: WNestedPageLayoutProps) => <WNestedPageLayoutInner {...props} />);
